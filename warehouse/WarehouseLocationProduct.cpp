...
{
	if (productId.empty()) {
		throw  std::exception("Id producktu jest wymagane");
	}
	
	this->ProductId = productId;
	if (volume <= 0) {
		throw  std::exception("Objetosc produktu nie moze byc mniejsza od zera");
	}
	this->Volume = volume;
	if (locationId.empty()) {
		throw  std::exception("Id lokalizaji jest wymagane");
	}
